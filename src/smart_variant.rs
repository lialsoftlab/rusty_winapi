...
struct AutoVariant(Cell<VARIANT>); 
    ...
    fn from(x: SmartVariant) -> Self {
        ...
        unsafe {
            match x {
                ...
                SmartVariant::Int2(x) => { *result.vtype_mut() = VT_I2 as u16; *result.data_mut().iVal_mut() = x; result }, // A 2-byte integer.
                SmartVariant::Int4(x) => { *result.vtype_mut() = VT_I4 as u16; *result.data_mut().lVal_mut() = x; result }, // A 4-byte integer.
                SmartVariant::Real4(x) => { *result.vtype_mut() = VT_R4 as u16; *result.data_mut().fltVal_mut() = x; result }, // A 4-byte real.
                SmartVariant::Real8(x)  => { *result.vtype_mut() = VT_R8 as u16; *result.data_mut().dblVal_mut() = x; result }, // An 8-byte real.
                //SmartVariant::Currency(x) => { *result.vtype_mut() = VT_CY as u16; *result.data_mut().cyVal_mut() = x as CY }, // Currency. (i64)
                SmartVariant::Date(x) => { *result.vtype_mut() = VT_DATE as u16; *result.data_mut().date_mut() = x; result }, // A date. (f64)               
                SmartVariant::Text(x) => { *result.vtype_mut() = VT_BSTR as u16; *result.data_mut().bstrVal_mut() = AutoBSTR::try_from(x).unwrap().into(); result }, // A string. 
                SmartVariant::IDispatch(x) => { *result.vtype_mut() = VT_DISPATCH as u16; *result.data_mut().pdispVal_mut() = x; result }, //An IDispatch pointer.
                SmartVariant::ErrorCode(x)=> {*result.vtype_mut() =  VT_ERROR as u16; *result.data_mut().scode_mut() = x; result }, // An SCODE value. (i32)
                SmartVariant::Bool(x)  => { *result.vtype_mut() = VT_BOOL as u16; *result.data_mut().boolVal_mut() = if x { -1 } else { 0 }; result }, //A Boolean value. True is -1 and false is 0. (i16)                
                SmartVariant::Variant(x) => { *result.vtype_mut() = VT_VARIANT as u16; *result.data_mut().pvarVal_mut() = x; result },  // A variant pointer.
                SmartVariant::IUnknown(x) => { *result.vtype_mut() = VT_UNKNOWN as u16; *result.data_mut().punkVal_mut() = x; result }, // An IUnknown pointer.
                ...
                SmartVariant::Int1(x) => { *result.vtype_mut() = VT_I1 as u16; *result.data_mut().cVal_mut() = x; result }, // A character. (i8)
                SmartVariant::UInt1(x) => { *result.vtype_mut() = VT_UI1 as u16; *result.data_mut().bVal_mut() = x; result }, // An unsigned character. (u8)
                SmartVariant::UInt2(x) => { *result.vtype_mut() = VT_UI2 as u16; *result.data_mut().uiVal_mut() = x; result }, // An unsigned short. (u16)
                SmartVariant::UInt4(x) => { *result.vtype_mut() = VT_UI4 as u16; *result.data_mut().ulVal_mut() = x; result }, // An unsigned long.  (u32)
                SmartVariant::Int(x) => { *result.vtype_mut() = VT_INT as u16; *result.data_mut().intVal_mut() = x; result }, // An integer. (i32)
                SmartVariant::UInt(x)	=> { *result.vtype_mut() = VT_UINT as u16; *result.data_mut().uintVal_mut() = x; result }, // An unsigned integer. (u32)
                //SmartVariant::Record(x) => { *result.vtype_mut() = VT_RECORD as u16; *result.data_mut().n4_mut() = x; result }, // A user-defined type.
                SmartVariant::Array(x) => { *result.vtype_mut() = VT_ARRAY as u16; *result.data_mut().parray_mut() = x; result }, // A SAFEARRAY pointer.
                SmartVariant::ByRef(x) => { *result.vtype_mut() = VT_BYREF as u16; *result.data_mut().byref_mut() = x; result }, // A void pointer for local use.
            }
        }
    }
}
